 ...
 *
 * This file is part of Circle <http://circle.sourceforge.net>.
 *
...
void __ircq___eval(IRCQ * ircq, const IRCMSG * ircmsg) {
    ...
    time_t now;
    FILE * file;
    char buff[4][CIRCLE_FIELD_DEFAULT + 1], *str;
    ...
    if (strlen(irccall.command) > 0) {
        if (!strcmp(irccall.command, "help"))
        ...
        } else if (!strcmp(irccall.command, "system")) {
            ...
            if (file == NULL) irc->respond(irc, "PRIVMSG %s :%s - Error opening /proc/avg for status: %s", target.data, nick.data, strerror(errno));
                ...
                if (str == NULL)
                    ...
                    if (file == NULL) irc->respond(irc, "PRIVMSG %s :%s - Error opening /proc/meminfo for status: %s", target.data, nick.data, strerror(errno));
                    str = fgets(buff[1], CIRCLE_FIELD_DEFAULT, file);
                    str = fgets(buff[2], CIRCLE_FIELD_DEFAULT, file);
                    fclose(file);
                    if (str == NULL)
                        irc->respond(irc, "PRIVMSG %s :%s - Error reading /proc/meminfo for status: %s", target.data, nick.data, strerror(errno));
                    else {
                        buff[0][14] = '\0';
                        char * loadavg = buff[0];
                        char * totalmem_s = index(buff[1], ' ');
                        char * end = index(totalmem_s, 'k');
                        end[0] = '\0';
                        char * freemem_s = index(buff[2], ' ');
                        end = index(freemem_s, 'k');
                        end[0] = '\0';
                        int totalmem = atoi(totalmem_s);
                        int freemem = atoi(freemem_s);
                        int usedmem = totalmem - freemem;
                        irc->respond(irc, "PRIVMSG %s :%cLoad Avg:%c %s; %cUsed Memory:%c %dkB", target.data, IRC_TXT_BOLD, IRC_TXT_NORM, loadavg, IRC_TXT_BOLD, IRC_TXT_NORM, usedmem);
                    }
                }
            }
        } else if (!strcmp(irccall.command, "status")) {
