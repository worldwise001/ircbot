...
void sighandle_parent(int sig)
{
	...
	if (globals.size > 0)
	{
		...
		for (i = 0; i < globals.size; i++)
			if (globals.config[i].pid != 0) kill(globals.config[i].pid, SIGUSR1);
...
void sighandle_child(int sig)
{
	...
	else
	{
		...
		if (globals.confPtr->enabled)
			write_data(globals.confPtr->sockfd, "QUIT :Terminated by signal\n");
		else
...
void sighandle_lib(int sig)
{
	if (sig == SIGSEGV)
	{
		...
		kill(globals.parent_pid, SIGTERM);
	}
	else if (sig != SIGUSR1)
		kill(globals.parent_pid, sig);
	else
...
int set_signals(int type)
{
	...
	switch(type)
	{
		...
		newaction.sa_flags = newaction.sa_flags;
		...
		newaction.sa_flags = newaction.sa_flags | SA_RESETHAND;
		if ((sigaction(SIGUSR1, &newaction, NULL) + sigaction(SIGABRT, &newaction, NULL) + sigaction(SIGTERM, &newaction, NULL) + sigaction(SIGINT, &newaction, NULL)) < 0)
		{
			irc_printf(IRCERR, "Error setting signal handlers: %s\n", strerror(errno));
			return -1;
		}
